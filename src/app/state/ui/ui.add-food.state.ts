...
export class UiAddFoodState {
  ...
  static allIngredientsOptions(state: IUiAddFoodModel) {
    let result = new Map<number, Partial<IPage<IFood> & { searchTerm: string }>>();
    for (let key of state.ingredientsOptions.keys()) {
      result.set(key, {
        ...state.ingredientsOptions.get(key),
        data: [
          ...state.ingredientsOptions.get(key)?.data,
          state.editingFood?.ingredients[key],
        ],
      });
    }
    return result;
